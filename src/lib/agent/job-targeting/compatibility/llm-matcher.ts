...
export const JOB_MATCHER_SYSTEM_PROMPT = [
  ...
  'Quando o requisito pedir uma ferramenta, produto, biblioteca, framework ou certificacao especifica, a evidencia precisa citar essa mesma familia nomeada para supported.',
  'Se a evidencia citar apenas uma ferramenta concorrente, uma ferramenta da mesma categoria ampla, ou uma capacidade generica sem a ferramenta especifica pedida, classifique como unsupported.',
  'Nunca use adjacent para uma ferramenta concorrente da mesma categoria. Ferramenta concorrente e unsupported, mesmo quando demonstra experiencia geral relacionada.',
  'Nunca use can_bridge_to_target_role para afirmar ou aproximar ferramenta concorrente; escolha unsupported e must_not_claim.',
  'Use adjacent apenas quando houver evidencia relacionada ao mesmo tipo de trabalho, tecnologia base ou responsabilidade, mas sem atender exatamente o requisito.',
  'Se o requisito pedir uma ferramenta/biblioteca/framework especifico e a evidencia citar uma tecnologia base, runtime, plataforma ou familia tecnica relacionada, mas nao o nome exato, classifique como adjacent e can_bridge_to_target_role.',
  'Nao trate tecnologia base relacionada como ferramenta concorrente; ferramenta concorrente e outro produto que resolve a mesma funcao de mercado.',
...
export function buildJobMatcherUserPrompt(requirement: MatcherRequirement, evidenceBullets: string[]): string {
  return [
    ...
    '',
    'Regras de permissao:',
    '- supported => can_claim_directly',
    '- adjacent => can_bridge_to_target_role',
    '- unsupported => must_not_claim',
    '',
    'Regras conservadoras adicionais:',
    '- Ferramenta especifica exige evidencia da mesma ferramenta/familia nomeada; ferramenta concorrente ou evidencia generica nao basta.',
...
function getDefaultOpenAIClient(): OpenAI {
  ...
  if (!apiKey) {
    throw new Error('Missing required environment variable OPENAI_API_KEY for job matcher LLM.')
  }
...
function retryDelayMs(attempt: number, config: JobMatcherRetryConfig): number {
  ...
  return config.retryJitter ? Math.round(base * (0.5 + Math.random())) : base
}
...
function isRateLimitError(error: unknown): boolean {
  return errorStatus(error) === 429 || /429|rate_limit_exceeded|rate limit|throttl/i.test(errorText(error))
}
...
function isProvider5xxError(error: unknown): boolean {
  ...
  return (typeof status === 'number' && status >= 500) || /5\d\d|temporarily_unavailable|unavailable|provider/i.test(errorText(error))
}
...
function exhaustedReasonForError(error: unknown): Exclude<LlmMatcherFallbackReason, 'classification_failed' | 'low_confidence_reclassified'> {
  if (isRateLimitError(error)) return 'rate_limit_retries_exhausted'
  if (isTimeoutError(error)) return 'llm_timeout_retries_exhausted'
...
export async function classifyRequirementWithLlm(input: {
  ...
  for (let attempt = 1; attempt <= retryConfig.maxRetries + 1; attempt += 1) {
    try {
      ...
      if (!parsed) {
        return conservativeFallback(input.requirement, input.resumeEvidence, 'classification_failed', latencyMs, retryCount)
      }

      const fallbackReason = parsed.confidence < confidenceThreshold
        ? 'low_confidence_reclassified' satisfies LlmMatcherFallbackReason
        : undefined
      const effectiveOutput = fallbackReason
        ? {
            evidenceLevel: 'unsupported',
            rewritePermission: 'must_not_claim',
      ...
      if (!isTransientProviderError(error) || attempt > retryConfig.maxRetries) {
        ...
        return conservativeFallback(input.requirement, input.resumeEvidence, reason, Date.now() - startedAt, retryCount)
      }
...
export async function classifyRequirementsWithLlm(input: {
  ...
  const classifications = await runWithConcurrencyLimit(
    ...
    async (requirement) => {
      try {
        return await classifyRequirementWithLlm({
          requirement,
          resumeEvidence: input.resumeEvidence,
          evidenceBullets,
