...
import { z } from "zod"
import { Button, Card, CardContent, CardFooter, CardHeader, Input, Label, Separator } from "../components/ui"
...
export default function LoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Mock success and redirect to dashboard
      navigate("/dashboard")
    } catch (err) {
      setError('root', { message: 'Erro ao entrar. Tente novamente.' })
    }
  }
      ...
      <Card className="w-full max-w-[400px] shadow-xl border-border/50">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <CardContent className="space-y-4">
            ...
            {errors.root && (
              <p className="text-sm text-destructive text-center font-medium">{errors.root.message}</p>
            )}
          </CardContent>
