...
import { Bot, User, Download, Check, X } from "lucide-react"
          ...
          <Card className="bg-card/50 border-border/50">
            <CardContent className="p-4 space-y-4">
              ...
              {analysisResult.missingKeywords.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground">Palavras-chave faltando (adicionadas)</p>
                  <div className="flex flex-wrap gap-1.5">
                    {analysisResult.missingKeywords.map((keyword, i) => (
                      <Badge key={i} variant="outline" className="text-xs gap-1 border-yellow-500/50 text-yellow-500">
                        <X className="h-3 w-3" />
                        {keyword}
                      </Badge>
                    ))}
